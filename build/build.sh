#!/bin/bash
#
# -- Build Apache Spark Standalone Cluster using Docker Images
...
SHOULD_BUILD_JUPYTERLAB="$(grep -m 1 build_jupyterlab build.yml | grep -o -P '(?<=").*(?=")')"
...
function buildImages() {
  ...
  if [[ "${SHOULD_BUILD_BASE}" == "true" ]]
  then
    docker build --build-arg build_date="${BUILD_DATE}" -f docker/base/Dockerfile -t base:latest .
  fi

  if [[ "${SHOULD_BUILD_SPARK}" == "true" ]]
  then

    docker build --build-arg build_date="${BUILD_DATE}" --build-arg spark_version="${SPARK_VERSION}" --build-arg hadoop_version="${HADOOP_VERSION}" \
  ...
  then
    docker build \
      --build-arg build_date="${BUILD_DATE}" --build-arg spark_version="${SPARK_VERSION}" --build-arg jupyterlab_version="${JUPYTERLAB_VERSION}" -f docker/jupyterlab/Dockerfile \
